...
def insert_label(curs,conn,picture_id,label):
    ...
    if presence == True:
        sql = """
        UPDATE labels
        SET picture_id = %s::INTEGER,label = %s, user_id = %s::TEXT
        WHERE picture_id = %s AND user_id = %s::TEXT;
        """
        param = [picture_id,label,current_user.id,picture_id,current_user.id]
        curs.execute(sql,param)
        conn.commit()
        if label == 'none' and q[-1] != 'none':
            return -1
        elif q[-1] == 'none' and label != 'none':
            return 1
        else:
            return 0
    else:
...
def get_wbc_label():
    ...
    n = insert_label(cur_wbc,conn_wbc,picture_id,label)
